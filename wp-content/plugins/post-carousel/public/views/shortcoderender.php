	...
	public function post_carousel_shortcode( $attributes ) {
	    ...
	    <script type="text/javascript">
				...
				jQuery("#sp-post-carousel-' . $post_id . '").slick({
                    ...
                    prevArrow: "<div class=\'slick-prev\'><i class=\'sp-pc-font-icon icon-angle-left\'>&#xf104;</i></div>",
                    nextArrow: "<div class=\'slick-next\'><i class=\'sp-pc-font-icon icon-angle-right\'>&#xf105;</i></div>",
                    slidesToScroll: 1,
				...
				while ( $que->have_posts() ) : $que->the_post();
					...
					if ( $pc_post_author == 'true' || $pc_post_date == 'true' ) {
						...
						if ( $pc_post_author == 'true' ) {
							$outline .= '<li><i class="sp-pc-font-icon icon-user">&#xe801;</i><a href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">' . esc_html( get_the_author() ) . '</a></li>';
						}
						if ( $pc_post_date == 'true' ) {
							$outline .= '<li><i class="sp-pc-font-icon icon-clock">&#xe802;</i>';
							$outline .= '<time class="entry-date published updated" datetime="' . esc_attr( get_the_date( 'c' ) ) . '">' . esc_html( date_i18n( get_option( 'date_format' ) ) ) . '</time></li>';
						}
						$outline .= '</ul></div>';
					}
					if ( $pc_post_content == 'true' ) {
						...
						$outline      .= "<p class='sp-pc-content'>" . $shortexcerpt . "</p>";
					}
